...
bool VisionSystem::init(int m_choice, int t_choice) {
    model_choice = m_choice;
    tracker_choice = t_choice;
    // Prepare model/tracker names
    modelName_   = (model_choice == 1 ? "Haar" : model_choice == 2 ? "SSD" : "LBP");
    trackerName_ = (tracker_choice == 1 ? "CSRT" : tracker_choice == 2 ? "KCF" : "MOSSE");
    ...
    if (!log_file_) {
        std::cerr << "[VISION] Nie można otworzyć pliku log: " << logPath << std::endl;
        return false;
    }
    // CSV header
    log_file_ << "TestID,Klatka,FPS,Tryb,Osoba,Confidence,Model,Tracker,TwarzCzas[ms],MarkerCzas[ms]\n";

    // Initialize timing
    last_time_ = Clock::now();

    // Paths
    std::string face_data_path      = "/home/piotrek/catkin_ws/src/face_marker_tracker/faces";
    std::string face_cascade_path  = "/usr/share/opencv4/haarcascades/haarcascade_frontalface_default.xml";

    // Load face detector
    if (model_choice == 1) { // Haar
        if (!face_cascade.load(face_cascade_path)) {
            std::cerr << "[VISION] Nie można załadować Haar Cascade!" << std::endl;
            return false;
        }
    } else if (model_choice == 2) { // SSD
        ssd_net = dnn::readNetFromCaffe(
            "../classifiers/deploy.prototxt",
            "../classifiers/res10_300x300_ssd_iter_140000.caffemodel"
        );
        if (ssd_net.empty()) {
            std::cerr << "[VISION] Nie można załadować SSD!" << std::endl;
            return false;
        }
    } else { // LBP
        if (!face_cascade.load("../classifiers/lbpcascade_frontalface.xml")) {
            std::cerr << "[VISION] Nie można załadować LBP Cascade!" << std::endl;
            return false;
        }
    }
    ...
    if (!images.empty()) face_model->train(images, labels);
    else std::cerr << "[VISION] Brak zdjęć do treningu LBPH!" << std::endl;

    return true;
}

VisionResult VisionSystem::process_frame(Mat& frame) {
    VisionResult result;
    ...
    if (!face_verified) {
        ...
        auto t1 = Clock::now();
        if (model_choice == 2) {
            Mat inputBlob = dnn::blobFromImage(frame, 1.0, Size(300,300), Scalar(104,177,123));
            ssd_net.setInput(inputBlob);
            Mat det = ssd_net.forward();
            ...
            face_cascade.detectMultiScale(gray, faces, 1.2, 5);
        }
        auto t2 = Clock::now();
        ...
        for (auto& face : faces) {
            ...
            face_model->predict(roi, label, conf);
        ...
        result.status_text = "Tryb: Wyszukiwanie twarzy";
        // Log and return
        if (log_file_) log_file_ << test_id_<<','<<frame_counter_<<','<<last_fps_<<','
            << result.status_text << ',' << result.recognized_name << ',' << result.confidence << ','
    ...
    if (face_verified && !is_tracking) {
        ...
        for (size_t i = 0; i < ids.size(); i++) {
            if (ids[i] == AUTHORIZED_MARKER_ID) {
                ...
                if      (tracker_choice==1) tracker = TrackerCSRT::create();
                else if (tracker_choice==2) tracker = TrackerKCF::create();
                else                         tracker = TrackerMOSSE::create();
        ...
        // timeout logic
        result.status_text = "Tryb: Twarz rozpoznana – szukam markera";
        if (log_file_) log_file_ << test_id_<<','<<frame_counter_<<','<<last_fps_<<','
            << result.status_text << ',' << result.recognized_name << ',' << result.confidence << ','
            << modelName_ << ',' << trackerName_ << ',' << face_time_ms << ',' << marker_time_ms << '\n';
        return result;
    ...
    // 3) Tracking
    if (is_tracking && tracker) {
        auto t5 = Clock::now();
        bool ok = tracker->update(frame, target_box);
        auto t6 = Clock::now();
        ...
        if (ok) {
            ...
            result.status_text     = "Tryb: Utrata śledzenia";
        }
        if (log_file_) log_file_ << test_id_<<','<<frame_counter_<<','<<last_fps_<<','
            << result.status_text << ',' << result.recognized_name << ',' << result.confidence << ','
            << modelName_ << ',' << trackerName_ << ',' << face_time_ms << ',' << marker_time_ms << '\n';
...
void VisionSystem::draw_status(Mat& frame, const VisionResult& result) {
    putText(frame, result.status_text, Point(10,30), FONT_HERSHEY_SIMPLEX, 0.8, Scalar(0,255,255), 2);
